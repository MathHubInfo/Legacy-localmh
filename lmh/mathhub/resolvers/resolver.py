...
class MathHubResolver(AbstractCaseClass):
    """ Represents a (local or remote) Resolver that can resolve specifications
    of repositories. """
    
    def can_answer_for(self, name: str) -> bool:
        """ Performs a partial check if this resolver can answer queries for
        the given instance name. If this method returns true, the
        MathHubResolver can answer queries, for all other cases the behaviour
        is unspecified. By default returns False, so it should be overridden in
        a subclass.
    ...
    def _match_name(self, spec: str, group: str, name: str) -> bool:
        """ Protected function used to check if a single name matches a
        specification. By default uses fnmatch.
    ...
    def _match_full(self, spec: str, group: str, name: str) -> bool:
        """
        Protected function used to check if a full repository name matches a
        specification. By default uses fnmatch.
    ...
    def _resolve_group(self, group: str) -> str:
        """ Protected function used to resolve a group name. By default does
        nothing.
    ...
    def get_repo_path(self, group: str, name: str) -> str:
        ...
        May also throw RepositoryNotFound. Should be overridden by the
        subclass.
    ...
    def get_all_repos(self) -> List[Tuple[str, str]]:
        ...
        NotImplementedError if not available. Should be overridden by the
        subclass.
    ...
    def get_all_groups(self) -> List[str]:
        """ Gets a (possibly cached) list of groups available on this
        MathHubResolver() instance or throws NotImplementedError. If not
        overridden by the subclass finds the groups by checking the output of
        get_all_repos().
        """
    ...
    def get_repos_in(self, group: str) -> List[Tuple[str, str]]:
        ...
        MathHubResolver() instance or throws NotImplementedError. May also
        throw GroupNotFound() if the given group does not exist. If not
        overriden by the subclass finds the groups by checking the output of
        get_all_groups() and get_all_repos().
    ...
    def get_repos_matching(self, *spec: List[str],
                           base_group: Optional[str] = None)\
            -> List[Tuple[str, str]]:
        """ Gets a (possibly cached and ordered) list of repositories matching
        the given specification. If not overridden by the subclass uses the
        output of get_all_repos() and get_repos_in().
        ...
        'group/name'. If empty and base_group is None, the full list
        of repositories will be returned. If empty and base_group has some
        value only repositories from that group will be returned.
    ...
    def get_repo_matching(self, spec: str, base_group: Optional[str] = None)\
            -> Tuple[str, str]:
        """
        Same as get_repos_matching() but returns only a single repository.
        Throws RepositoryNotFound() if not repository exists. If not overridden
        by the subclass uses the output of get_repo_matching().

        :param spec: A string or pattern that can contains *s that will be
    ...
    def repo_exists(self, group: str, name: str) -> bool:
        """ Checks if this Resolver can find the path to a repository. Should
        be overridden by subclass for speed advantages. By default checks the
        output of get_all_repos().
...
class RepositoryNotFound(exceptions.MathHubException):
    ...
    def __init__(self):
        ...
        super(RepositoryNotFound, self).__init__('Repository not found on' +
                                                 ' this MathHubResolver()' +
                                                 ' instance. ')


class GroupNotFound(exceptions.MathHubException):
    """ Exception that is thrown when a group is not found on a
    MathHubResolver() instance. """

    def __init__(self):
        """ Creates a new GroupNotFound() instance. """

        super(GroupNotFound, self).__init__('Group not found on this ' +
                                            'MathHubResolver() instance. ')

__all__ = ["MathHubResolver", "RepositoryNotFound", "GroupNotFound"]
